...
### Cohérence Spatiale de l'Environnement (CRITIQUE)

Les images de l'histoire seront vues comme un diaporama. Les spectateurs remarqueront si les meubles, objets ou l'agencement de la pièce changent de position entre les images situées au même endroit. Pour éviter cela :

- **ÉTABLIR UNE DISPOSITION SPATIALE :** Le PREMIER `imagePrompt` situé dans un lieu donné DOIT définir une disposition spatiale claire et spécifique de l'environnement. Décris où les objets clés sont placés par rapport à la caméra : gauche, droite, centre, premier plan, arrière-plan. Par exemple : "A cozy bedroom seen from a slightly elevated angle — a small wooden bed with a blue blanket on the LEFT side of the room, a round window with yellow curtains on the BACK WALL, a bookshelf filled with colourful books on the RIGHT, and a braided rug on the wooden floor in the CENTER."
- **RÉPÉTER LA DISPOSITION TEXTUELLEMENT :** Chaque `imagePrompt` ultérieur situé au MÊME endroit DOIT répéter la même description spatiale de l'environnement — mêmes objets, mêmes positions (gauche/droite/centre/premier plan/arrière-plan), même disposition relative. Copie la description de la disposition du cadre d'établissement et réutilise-la. Seules les positions des personnages, les actions et les petits détails du premier plan doivent changer.
- **UTILISER DES DIRECTIONS EXPLICITES :** Décris toujours le placement des objets en utilisant des termes spatiaux explicites : "on the left", "on the right", "in the center", "against the back wall", "in the foreground", "in the far corner". Ne laisse jamais le placement des objets ambigu ou implicite.
- **LES NOUVEAUX LIEUX REÇOIVENT DE NOUVELLES DISPOSITIONS :** Quand l'histoire se déplace vers un NOUVEAU lieu, rédige une nouvelle disposition spatiale pour ce lieu. Puis maintiens-la de manière cohérente pour toutes les images suivantes dans ce lieu, en suivant les mêmes règles ci-dessus.
- **COHÉRENCE DE L'ANGLE DE CAMÉRA :** Lorsque des images consécutives partagent le même lieu, garde le même angle et la même perspective de caméra, sauf s'il y a une raison narrative forte de les changer. Un angle de caméra cohérent renforce l'impression que les objets restent à leur place.
