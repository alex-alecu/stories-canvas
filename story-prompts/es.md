...
### Consistencia Espacial del Entorno (CRÍTICO)

Los fotogramas de la historia se verán como una presentación de diapositivas. Los espectadores notarán si los muebles, objetos o la disposición de la habitación cambian de posición entre fotogramas ubicados en el mismo lugar. Para prevenir esto:

- **ESTABLECE UNA DISPOSICIÓN ESPACIAL:** El PRIMER `imagePrompt` ambientado en una ubicación determinada DEBE definir una disposición espacial clara y específica del entorno. Describe dónde están colocados los objetos clave en relación con la cámara: izquierda, derecha, centro, primer plano, fondo. Por ejemplo: "A cozy bedroom seen from a slightly elevated angle — a small wooden bed with a blue blanket on the LEFT side of the room, a round window with yellow curtains on the BACK WALL, a bookshelf filled with colourful books on the RIGHT, and a braided rug on the wooden floor in the CENTER."
- **REPITE LA DISPOSICIÓN TEXTUALMENTE:** Cada `imagePrompt` posterior ambientado en la MISMA ubicación DEBE repetir la misma descripción espacial del entorno — mismos objetos, mismas posiciones (izquierda/derecha/centro/primer plano/fondo), misma disposición relativa. Copia la descripción de la disposición del fotograma de establecimiento y reutilízala. Solo las posiciones de los personajes, acciones y pequeños detalles del primer plano deben cambiar.
- **USA DIRECCIONES EXPLÍCITAS:** Describe siempre la ubicación de los objetos usando términos espaciales explícitos: "on the left", "on the right", "in the center", "against the back wall", "in the foreground", "in the far corner". Nunca dejes la ubicación de los objetos ambigua o implícita.
- **NUEVAS UBICACIONES RECIBEN NUEVAS DISPOSICIONES:** Cuando la historia se mueve a una ubicación NUEVA, escribe una nueva disposición espacial para esa ubicación. Luego mantenla consistente para todos los fotogramas posteriores en esa ubicación, siguiendo las mismas reglas anteriores.
- **CONSISTENCIA DEL ÁNGULO DE CÁMARA:** Cuando fotogramas consecutivos comparten la misma ubicación, mantén el ángulo y la perspectiva de la cámara iguales a menos que haya una razón narrativa fuerte para cambiarlo. Un ángulo de cámara consistente refuerza la sensación de que los objetos permanecen en su lugar.
