-- Count speech requests in audio totals. Preserve historical usage and wallet debits.
    ...
    SELECT
      ...
      COALESCE(SUM(cost_usd_micros) FILTER (
        WHERE provider IN ('openrouter', 'openai', 'gemini')
          AND operation NOT IN ('character_sheet', 'page_image', 'page_audio')
      ...
      COALESCE(SUM(cost_usd_micros) FILTER (WHERE provider = 'elevenlabs' OR operation = 'page_audio'), 0)::BIGINT AS audio_cost_usd_micros
    ...
    SELECT
      ...
      SUM(e.cost_usd_micros) FILTER (
        WHERE e.provider IN ('openrouter', 'openai', 'gemini')
          AND e.operation NOT IN ('character_sheet', 'page_image', 'page_audio')
      ...
      SUM(e.cost_usd_micros) FILTER (WHERE e.provider = 'elevenlabs' OR e.operation = 'page_audio')::BIGINT AS audio_cost_usd_micros
    FROM stories s
